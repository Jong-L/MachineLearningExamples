...
在这里已经进入增量学习，之前的策略迭代和值迭代是批量学习，在访问完所有状态后，再进行更新
"""
...
def td_state_value(env: GridWorld, config: TDConfig) -> TDResult:
    ...
    for count in range(config.max_iteration):
        ...
        for t in range(config.max_episode_length):
            state_next,action,reward=env.sample_next(env.index_to_state(state_idx),env.policy,rng)#采用环境默认的随机策略
        ...
        erros.append(rmse)
        if rmse < config.threshold:
            print(f"Convergence achieved at iteration {count}")
            break
...
if __name__ == "__main__":
    ...
    print("\n=== 运行配置与耗时 ===")
    print("算法: TD(0) State Value (with threshold stop)")
